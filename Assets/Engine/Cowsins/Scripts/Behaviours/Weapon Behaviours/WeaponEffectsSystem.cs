...
namespace cowsins
{
    public class WeaponEffectsSystem
    {
        ...
        public void RegisterWeaponPools()
        {
            ...
            if (weaponReference.Weapon.bulletGraphics != null)
        ...
        public void SpawnBulletShells()
        {
            ...
            foreach (var p in context.firePoint)
            {
                ...
                if (weaponReference.Weapon.showBulletShells && (int)weaponReference.Weapon.shootStyle != 2)
                {
                    var bulletShell = PoolManager.Instance.GetFromPool(weaponReference.Weapon.bulletGraphics.gameObject, p.position, Quaternion.identity);
